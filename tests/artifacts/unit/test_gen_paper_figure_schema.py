...
from open_notebook.artifacts.generators.paper_figure import (
    DataPointSchema, PaperFigureGenerator, PaperFigureSchema, SeriesSchema,
)
...
class TestPaperFigureSchema:
    ...
    def test_chart_type_must_be_literal(self):
        fig = PaperFigureSchema(
            title="Figure 1", chart_type="bar", x_label="Category", y_label="Value",
            series=[SeriesSchema(name="S", data=[DataPointSchema(x="A", y=1.0)])],
    ...
    def test_schema_rejects_invalid_chart_type(self):
        with pytest.raises(Exception):
            PaperFigureSchema(
                title="Bad", chart_type="pie",  # type: ignore
                x_label="x", y_label="y", series=[], caption="nope",
            )

    def test_schema_validates_scatter(self):
        fig = PaperFigureSchema(
            title="Scatter", chart_type="scatter", x_label="X", y_label="Y",
            series=[SeriesSchema(name="data", data=[
                DataPointSchema(x=1.0, y=2.0), DataPointSchema(x=3.0, y=4.0),
            ])],
    ...
    def test_schema_round_trips(self):
        fig = PaperFigureSchema(
            title="Line", chart_type="line", x_label="Year", y_label="Revenue",
