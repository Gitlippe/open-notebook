...
from open_notebook.artifacts.generators.data_tables import (
    DataTablesGenerator, DataTablesSchema, TableSchema,
    _render_markdown_table, _render_markdown_all,
)
...
class TestDataTablesSchema:
    ...
    def test_table_requires_minimum_two_columns(self):
        with pytest.raises(Exception):
            TableSchema(title="Bad", columns=["Only one"], rows=[["value"]])
    ...
    def test_schema_validates_single_table(self):
        dt = DataTablesSchema(tables=[
            TableSchema(
                title="Accuracy Comparison",
                columns=["Model", "AIME 2024", "Cost"],
                rows=[["DeepSeek-V3.1", "80.0%", "$0.10"], ["TF-GRPO", "82.7%", "$0.001"]],
    ...
    def test_schema_allows_mixed_types_in_rows(self):
        t = TableSchema(
            title="Mixed", columns=["Name", "Score", "Pass"],
            rows=[["Alice", 95.5, True], ["Bob", 42, False]],
        )
        assert t.rows[0][1] == 95.5

    def test_schema_round_trips(self):
        dt = DataTablesSchema(tables=[TableSchema(title="RT", columns=["K", "V"], rows=[["key", "value"]])])
...
class TestMarkdownRenderer:
    def test_render_single_table_has_headers(self):
        t = TableSchema(title="Test Table", columns=["Col A", "Col B"], rows=[["val1", "val2"]])
    ...
    def test_render_table_includes_caption(self):
        t = TableSchema(title="T", columns=["A", "B"], rows=[], caption="From source X.")
        md = _render_markdown_table(t)
